...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getMajorName(major) {
    return major.name_ar || major.name || major.name_en || "تخصص غير محدد";
  }
  ...
  function getFacultyId(major) {
    const id = Number(major.faculty_id ?? major.facultyId);
  ...
  function showError(message) {
    ...
    pageErrorText.textContent =
      message || "حدث خطأ أثناء الاتصال بالخادم. حاول تحديث الصفحة.";
  ...
  function createMajorCard(major) {
    ...
    article.dataset.facultyId = String(getFacultyId(major) ?? "");

    article.dataset.search = normalizeText(
      [
        name,
        facultyName,
        universityName,
  ...
  function buildFacultyFilters() {
    const currentAllButton = facultyFilters.querySelector(
      '[data-filter="all"]',
    );

    facultyFilters.innerHTML = "";

    const allButton = currentAllButton || document.createElement("button");
    ...
    [...faculties.entries()]
      ...
      .forEach(([facultyId, facultyName]) => {
        ...
        button.type = "button";
        button.dataset.filter = String(facultyId);

        button.textContent = facultyName;
  ...
  function setActiveFilter(button) {
    facultyFilters.querySelectorAll(".filter-button").forEach((item) => {
      item.classList.toggle("active", item === button);
    });

    activeFaculty = button.dataset.filter || "all";
  ...
  function updateResults() {
    ...
    majorsGrid.querySelectorAll(".major-card").forEach((card) => {
      ...
      const matchesFaculty =
        activeFaculty === "all" || facultyId === activeFaculty;

      const visible = matchesSearch && matchesFaculty;
    ...
    resultsCount.textContent = `${visibleCount.toLocaleString("ar-YE")} تخصص`;
  ...
  function renderMajors() {
    ...
    allMajors.forEach((major) => {
      fragment.appendChild(createMajorCard(major));
    });
  ...
  async function loadMajors() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.message || `فشل طلب التخصصات (${response.status})`,
        );
      }

      const majors = extractRows(result);

      allMajors = majors.filter((major) => major && major.is_active !== false);
      ...
      showError(error.message || "تعذر تحميل التخصصات من الخادم.");
    }
  }
  ...
  function getLocalFavorites() {
    try {
      const stored = JSON.parse(
        localStorage.getItem("muwajeh_favorite_majors") || "[]",
      );
  ...
  function saveLocalFavorites(ids) {
    localStorage.setItem("muwajeh_favorite_majors", JSON.stringify([...ids]));
  }

  function applyFavoriteState() {
    const favorites = getLocalFavorites();

    majorsGrid.querySelectorAll(".favorite-button").forEach((button) => {
      const id = Number(button.dataset.majorId);
      ...
      icon.classList.toggle("fa-solid", selected);

      icon.classList.toggle("fa-regular", !selected);
    });
  }

  facultyFilters.addEventListener("click", (event) => {
  ...
  majorsGrid.addEventListener("click", (event) => {
  ...
  retryButton.addEventListener("click", loadMajors);
