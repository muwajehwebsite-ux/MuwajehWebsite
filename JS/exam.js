...
async function checkExistingExam() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Assessment status failed: ${response.status}`);
    }

    const result = await response.json();

    if (!result.success) {
      throw new Error(result.message || "Could not check assessment");
    }
    ...
    if (result.status === "in_progress") {
      attemptId = Number(result.data.attempt.id);

      localStorage.setItem(ATTEMPT_STORAGE_KEY, String(attemptId));
    ...
    showToast("تعذر التحقق من حالة الاختبار. تأكد من تشغيل الخادم.", "error");
...
async function startNewAttempt() {
  ...
  try {
    const response = await fetch(`${API_URL}/assessments/attempts`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    if (!response.ok) {
      throw new Error(`Attempt creation failed: ${response.status}`);
    }

    const result = await response.json();

    if (!result.success || !result.data || !result.data.id) {
      throw new Error(result.message || "Invalid attempt response");
    }

    attemptId = Number(result.data.id);
    ...
    showToast("تعذر بدء الاختبار.", "error");
...
function renderQuestion() {
  ...
  answerOptions.forEach((button) => {
    ...
    button.classList.toggle("selected", Number(savedAnswer) === value);
  });
  ...
  progressPercentage.textContent = `${progress}% مكتمل`;
...
async function saveAnswerToDatabase(questionId, answerValue) {
  const token = getToken();
  ...
  if (!response.ok) {
    ...
    throw new Error(
      result?.message || `Answer save failed: ${response.status}`,
    );
  }
...
answerOptions.forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    try {
      await saveAnswerToDatabase(question.id, value);
    } catch (error) {
      console.error("Could not save answer:", error);

      showToast("تعذر حفظ الإجابة. يرجى المحاولة مرة أخرى.", "error");
    }
  });
});
...
nextButton.addEventListener("click", async () => {
  ...
  if (selectedAnswer === undefined) {
    showToast("يرجى اختيار إجابة قبل الانتقال إلى السؤال التالي.", "error");
...
async function finishExam() {
  if (!areAllQuestionsAnswered()) {
    showToast("يرجى الإجابة عن جميع الأسئلة قبل إنهاء الاختبار.", "error");

    return;
  }

  if (!attemptId) {
    showToast("لم يتم العثور على محاولة الاختبار.", "error");
  ...
  nextButton.querySelector("span").textContent = "جاري الحفظ...";
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.message || "Failed to finish assessment");
    }
    ...
    showToast(error.message || "تعذر حفظ نتيجة الاختبار.", "error");

    nextButton.disabled = false;

    nextButton.querySelector("span").textContent = "إنهاء";
  }
}

async function initializeExam() {
  const canStart = await checkExistingExam();
