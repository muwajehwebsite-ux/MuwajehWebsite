...
function renderResults(results) {
  ...
  results.slice(0, 6).forEach(function (result, index) {
    ...
    scoreRing.className = `score-ring score-${Math.round(score)}`;

    scoreRing.innerHTML = `<span>${Math.round(score)}%</span>`;
    ...
    if (index === 0) {
      ...
      label.innerHTML = `<i class="fa-solid fa-star"></i> توافق ممتاز`;
